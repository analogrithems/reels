...
- **Replace audio** — substitute another audio file for the clip’s sound (i.e. mute the primary clip *and* insert the new audio on a dedicated lane in one step). Export-side mix is ready (**U2-b** `amix`); remaining work is the Edit-menu UI.
- **Overlay audio** — **shipped** as **Edit → Overlay Audio…** (see **Project & timeline** above). Follow-on: **per-lane gain** / independent volume vs the source.
- **Partial-clip mute** — **shipped** as silence substitution on the export thread (see **Mute Clip Audio** under **Project & timeline** above). Per-clip **gain** (and duck/boost without full mute) remains open.
