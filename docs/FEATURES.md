...
- **Replace audio** — substitute another audio file for the clip’s sound (requires **U2-b** mixer work).
- **Overlay audio** — mix in an additional track with **independent volume** vs the source (requires **U2-b** mixer work).
- **Partial-clip mute** — silence substitution for mixed muted/unmuted timelines (today **Mute Clip Audio** only takes effect when every primary-track clip is muted; see **Project & timeline** above).
