...
5. **U2-e — Audio (Edit menu):** **Remove audio** **shipped** as per-clip **Mute Clip Audio** toggle (export emits `-an` when every primary-track clip is muted). **Replace** / **Overlay** now unblocked by **U2-b-export amix**: the dispatch already handles N≥2 audio lanes, so the remaining work is UI (a menu entry that inserts a replacement / overlay clip onto a dedicated audio lane) plus per-clip gain. Preview-side mix is still first-lane-only until the audio-thread rewrite lands.
