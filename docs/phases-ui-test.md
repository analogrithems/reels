...
### Status (last updated 2026-04-18)
...
| Test | What it covers |
|------|----------------|
| `app_window_matches_golden_png` | Full `AppWindow` at 400×320; `Window::take_snapshot()`; byte-for-byte RGBA compare to `tests/golden/app_window_default.png` |
...
**Refresh goldens** after intentional UI changes:
...
1. **Context:** Point at workspace `Cargo.toml`, `crates/reel-app/src/lib.rs` (test modules + `prepare_export_job`), `crates/reel-app/tests/ui_visual_golden.rs`, and `crates/reel-app/ui/app.slint`.
