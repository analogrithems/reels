...
| `make run` | `cargo run -p reel-app`; session log `reels.session.*.log` is written under the **directory you ran `make` from** (`REEL_LOG_SESSION_DIR`). Optional: `make run ARGS='path/to/media-or-project'` opens that file on launch (forwards to `reel [<path>]`). |
