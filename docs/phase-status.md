...
- [~] **Playback / export — Subtitles** — **SRT + WebVTT + TTML import → burn-in at export** shipped (**File → Insert Subtitle…**, `Ctrl+Alt+I`; SRT/WebVTT share one parser that handles `HH:MM:SS[.,]mmm` and WebVTT's `MM:SS.mmm`, cue settings, `WEBVTT` / `NOTE` / `STYLE` blocks; **TTML** (`.ttml` / `.dfxp` / `.xml`) rides a sibling parser (`reel_core::media::ttml`) that handles clock-time (`HH:MM:SS.fff`), offset-time (`h` / `m` / `s` / `ms`), namespace-prefixed `<tt:p>` tags, `<br/>` → `\n`, XML entity decoding, and `dur` as an alternative to `end`; `reel_core::parse_subtitle_file` / `probe_subtitle_file` dispatch on extension so insert-subtitle and the overlay cache stay format-agnostic; export appends `subtitles='<path>'` to the `-vf` chain so captions are rendered on the final oriented/scaled frame — TTML burn-in rides libass via the same filter and works for files with basic styling; heavily-styled TTML may fall through to a preview-only state while the file still parses). **Live preview of cue text shipped** — a bottom-centred caption strip overlays the preview video whenever a `TrackKind::Subtitle` lane has an active cue at the current playhead; cues are cached per source file (`subtitle::SubtitleCueCache`) so scrub / nudge / edit paths fire a single lookup, and a 100 ms Slint `Timer` drives the overlay during playback. Inter-cue gaps blank the overlay (inclusive start, exclusive end — matches how ffmpeg's renderer draws adjacent cues). **Remaining:** **ASS/SSA** styling, subtitle-as-stream mux (soft subtitles), and a per-lane **Subtitle Track** picker (today the first `TrackKind::Subtitle` lane with a covering cue wins) — all still open (see `MEDIA_FORMATS.md`)
...
- [x] **View** (optional) — **fullscreen** on playback toolbar **shipped** (icon button on the floating transport chrome next to Volume; toggles `window-fullscreen` via `view-toggle-fullscreen`, accessible-labelled "Enter fullscreen" / "Exit fullscreen" based on state). **Zoom to Video** is exposed as **Window → Fill** (sets `video-fit-mode = 1` — cover — so the frame fills the preview rect, cropping edges when aspect mismatches). **Pan when zoomed** **shipped**: when the scaled image overflows the preview rect on either axis (`View → Zoom In` past fit, or **Actual Size** on a ≥ viewport-sized source), left-button drag on the preview shifts the image via `preview-pan-x` / `preview-pan-y` (logical pixels, ephemeral, not saved to prefs). The drag lives on the existing wake-chrome TouchArea so transport chrome still surfaces on hover; cursor switches to `grab` / `grabbing` while pannable. Offsets are clamped inside `preview-clip` to `[-(img-w-vw)/2, (img-w-vw)/2]` (and same for y) so the image edge halts at the preview edge — raw `preview-pan-*` is held verbatim so zooming out and back in doesn't snap to 0, but the rendered offset stays bounded. Pan resets to 0 on **Zoom to Fit**, **Actual Size**, and all **Window → Fit / Fill / Center** entries
...
- [x] **Accessibility audit** (**U4**) — shipped. Labeled: every transport-bar TouchArea (skip-start / rewind / play / fast-forward / skip-end / speed / loop / mute / fullscreen / tools) carries `accessible-role: button;` + a stateful `accessible-label` (play/pause, mute/unmute, loop-on/off, fullscreen enter/exit, tools open/close); volume **Slider** and main timeline scrub **Slider** carry `accessible-label`; In/Out trim drag handles (`in-handle-touch`, `out-handle-touch`) labeled "In marker — drag to set trim start" / "Out marker — drag to set trim end"; the three timeline add-track buttons (`track-add-video` / `track-add-audio` / `track-add-subtitle`) labeled "Add video track" / "Add audio track" / "Add subtitle track"; per-lane filmstrip trim handles (`left-trim` / `right-trim`) labeled "Trim clip start — drag" / "Trim clip end — drag"; lane-delete trash (`del-ta`) labeled "Delete track"; all **10 export preset rows** labeled. **Sheet Confirm / Cancel pairs** now carry explicit `accessible-label` that names the action being confirmed rather than the bare button text — **Trim Clip…** ("Cancel trim and close sheet" / "Apply trim with entered begin and end seconds"), **Audio Lane Gain…** ("Cancel audio lane gain change and close sheet" / "Apply gain to selected audio lane"), **Resize Video…** ("Cancel resize and close sheet" / "Apply resize with entered scale percent"). The six **Resize** preset % buttons (25 / 50 / 75 / 100 / 150 / 200) also carry contextual labels ("Set resize scale to 25 percent", …, "Set resize scale to 100 percent (identity)", …) so a screen-reader user knows what each bare "%" button does. Slint 1.16 accepts `accessible-*` on `TouchArea`, `Slider`, and `Button`.
