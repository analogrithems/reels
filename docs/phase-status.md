...
- [x] **Timeline** — **two markers** on the seek bar (in/out range) — **Edit → Set In/Out Point** + **Clear Range Markers**; keys **I** / **O** / **Alt+X**; cyan/magenta overlay with tinted range on the slider; ephemeral per session (clears on close / new project). **Follow-on:** wire the range through export / batch ops (today it's state-only).
